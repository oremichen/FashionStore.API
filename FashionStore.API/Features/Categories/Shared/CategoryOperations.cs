...
public sealed class CategoryOperations(ICategoryRepository repository, ILogger<CategoryOperations> logger)
{
    public async Task<ResponseResult<IReadOnlyList<CategoryResponse>>> GetCategoriesAsync(CancellationToken cancellationToken)
    {
        ...
        return new ResponseResult<IReadOnlyList<CategoryResponse>>().Success(
            (await repository.GetPublicCategoriesAsync(cancellationToken)).Select(ToResponse).ToList(), "Categories retrieved successfully.");
    }
    ...
    public async Task<ResponseResult<IReadOnlyList<CategoryResponse>>> GetCategoriesWithParentAsync(CancellationToken cancellationToken)
    {
        ...
        return new ResponseResult<IReadOnlyList<CategoryResponse>>().Success(
            (await repository.GetCategoriesWithParentAsync(cancellationToken)).Select(ToResponse).ToList(), "Categories with a parent retrieved successfully.");
    }
    ...
    public async Task<ResponseResult<CategoryDetailsResponse>> UpdateAsync(string id, UpdateCategoryRequest request, CancellationToken cancellationToken)
    {
    ...
    private static CategoryResponse ToResponse(CategoryListItem category) => new()
    {
        Id = category.Id, Name = category.Name, Slug = category.Slug,
        ParentId = category.ParentId, HasSubCategory = category.HasSubCategory
    };
